...
from JITSmart.concat.model import Model
from JITSmart.my_util import convert_examples_to_features, TextDataset, eval_result, preprocess_code_line, \
...
def set_seed(args):
    ...
    # torch.cuda.manual_seed_all(args.seed)  # All GPU
    ...
    os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':16:8'  # 在cuda 10.2及以上的版本中，需要设置以下环境变量来保证cuda的结果可复现
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (inputs_ids, attn_masks, manual_features, labels, line_ids, line_label) = [x.to(args.device) for x in batch]
            model.train()
            loss, logits, _, _ = model(inputs_ids, attn_masks, manual_features, labels, line_ids, line_label)
            if args.n_gpu > 1:
                loss = loss.mean()
            ...
            global_step += 1
            if (step + 1) % args.save_steps == 0:
                results = evaluate(args, model, tokenizer, eval_when_training=True, eval_dataset=eval_dataset)
...
def evaluate(args, model, tokenizer, eval_when_training=False, eval_dataset=None):
    # build dataloader
    ...
    # else:
    #     eval_dataset = TextDataset(tokenizer, args, file_path=args.eval_data_file, mode='valid', buggy_lines_file_path=args.buggy_lines_file)
    ...
    for batch in eval_dataloader:
        (inputs_ids, attn_masks, manual_features, labels, line_ids, line_label) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, _, _ = model(inputs_ids, attn_masks, manual_features, labels, line_ids, line_label)
...
def test(args, model, tokenizer, best_threshold=0.5, eval_dataset=None):
    # build dataloader
    ...
    #     eval_dataset = pickle.load(open(cache_dataset, 'rb'))
    # else:
    #     eval_dataset = TextDataset(tokenizer, args, file_path=args.test_data_file, mode='test', buggy_lines_file_path=args.buggy_lines_file)
    #     pickle.dump(eval_dataset, open(cache_dataset, 'wb'))

    eval_sampler = SequentialSampler(eval_dataset)
    # eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=0, pin_memory=True,worker_init_fn=worker_init_fn)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=8,
                                 pin_memory=True, worker_init_fn=worker_init_fn)
    # multi-gpu evaluate
    if args.n_gpu > 1:
    ...
    for batch in tqdm(eval_dataloader):
        (inputs_ids, attn_masks, manual_features, labels, line_ids, line_label) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, attn_weights, line_dl_score = model(inputs_ids, attn_masks, manual_features, labels, line_ids,
                                                             line_label, output_attentions=True)
            ...
            line_scores.append(line_dl_score.cpu().numpy())
            # line_dl_label_list.append(line_label.cpu().numpy())

        nb_eval_steps += 1

    # with torch.no_grad():
    #     for batch in tqdm(eval_dataloader):
    #         (inputs_ids2, attn_masks2, manual_features2, labels2, line_ids2, line_label2) = [x.to(args.device) for x in batch]
    #
    #         loss, logit2, attn_weights, line_dl_score, outs2 = model(inputs_ids2, attn_masks2, manual_features2, labels2, line_ids2, line_label2, output_attentions=True)
    #         break
    ...
    # print(logit2)
    # print(logit1==logit2)
    # print(inputs_ids1 == inputs_ids2)
    # print(attn_masks1 == attn_masks2)
    # print(manual_features1 == manual_features2)
    # print(labels1 == labels2)
    # print(line_ids1 == line_ids2)
    # print(line_label1 == line_label2)
    # assert 1==2

    # output result
    # calculate scores
    logits = np.concatenate(logits, 0)  # (nums, 1)
    y_trues = np.concatenate(y_trues, 0)  # (nums, )
    attns = np.concatenate(attns, 0)  # (nums, 12, 512)
    line_scores = np.concatenate(line_scores, 0)  # (nums, 2)

    print('logits.shape: ', logits.shape)
    print('y_trues.shape: ', y_trues.shape)
    print('attns.shape: ', attns.shape)
    print('line_scores.shape', line_scores.shape)
    ...
    # 一个样本一个样本测试，最后所有样本的指标取平均
    for example, pred, prob, attn, line_sc in zip(eval_dataset.examples, y_preds, logits[:, -1], attns, line_scores):
        ...
        if int(example.label) == 1 and int(pred) == 1 and '[ADD]' in example.input_tokens:
            ...
            cur_line_sc = line_sc[:code_line_nums]
            cur_line_sc = cur_line_sc.tolist()

            line_idx = list(range(len(cur_line_sc)))
            cur_line_sc = pd.DataFrame({'idx': line_idx, 'han_line_score': cur_line_sc})

            # print('commit id: ', example.commit_id)
            # print('code_line_nums: ', code_line_nums)
...
def deal_with_attns(item, attns, pred, commit2codes, idx2label, idx2linescore, only_adds=False):
    '''
    ...
    # 将每个token的注意力分数分配到对应行，然后每行的缺陷得分=所有token注意力得分之和
    result_df = pd.merge(commit2codes, attn_df, how='left', on='token')
    result_df = result_df.groupby(['idx']).sum()  # 每行的缺陷得分=所有token注意力得分之和
    ...
    result_df['han_line_score'].fillna(0, inplace=True)

    IFA, top_20_percent_LOC_recall, effort_at_20_percent_LOC_recall, top_10_acc, top_5_acc = get_line_level_metrics(
        result_df['score'].tolist(), result_df['label'].tolist(), result_df['han_line_score'].tolist())
...
def main(args):
    ...
    if args.do_train:
        ...
        for idx, example in enumerate(train_dataset.examples[:1]):
            ...
            logger.info(f'line_ids_shape: {torch.tensor(example.line_ids).shape}')
            logger.info(f'line_label_shape: {torch.tensor(example.line_label).shape}')
    ...
    if args.do_test:
        ...
        test(args, model, tokenizer, best_threshold=0.5, eval_dataset=test_dataset)
